...
func (this *UgcClassifyController) Get(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	var list *models.TblUgcClassifyData
	var list_c *models.TblUgcCCountData
	if input.Para.Unit != "" {
		err, list_c = new(models.TblUgcClassify).GetUgcCCount(&input.Para)
		if err != nil {
			panic(fmt.Sprintf("TblUrgency GetLast error:%s", err.Error()))
			return
		}
	} else {
		err, list = new(models.TblUgcClassify).GetUgcClassify(&input.Para)
		if err != nil {
			panic(fmt.Sprintf("TblUrgency GetLast error:%s", err.Error()))
			return
		}
...
type UgcClassifyGetInput struct {
	Para models.TblUgcClassifySearchPara
}
