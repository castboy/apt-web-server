...
type UgcDController struct{}

var UgcDObj = new(UgcDController)

func (this *UgcDController) Get(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	//check ssn
	//TODO
	//parse parament
	input := UgcDGetInput{}
	...
	err, list := new(urgency.TblUgcD).GetUrgencyDetails(&input.Para)
	if err != nil {
		panic(fmt.Sprintf("TblUrgency GetLast error:%s", err.Error()))
		return
	}
	//check if abnormal connection alert info has already been added whitelist
	err1 := urgency.UgcAbnConnCheckIfInWL(list)
	if err1 != nil {
		panic(fmt.Sprintf("TblUrgency GetLast error1:%s", err1.Error()))
		return
	}
	//response
...
type UgcDGetInput struct {
	Para urgency.TblUgcDSearchPara
}
