...
import (
	...
	"apt-web-server_v2/models/scanEvent"
	"fmt"
	"net/http"
	"net/url"
	"strconv"
...
type SEDontroller struct{}

var SEDObj = new(SEDontroller)

func (this *SEDontroller) Get(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	//check ssn
	//TODO
	//parse parament
	input := SEDGetInput{}
	...
	err, list := new(scanEvent.TblSED).GetScanEventDetails(&input.Para)
	if err != nil {
		panic(fmt.Sprintf("TblScanEventDetails GetLast error:%s", err.Error()))
		return
	}
	//response
...
type SEDGetInput struct {
	Para scanEvent.TblSEDSearchPara
}
